...
if ( function_exists( 'lazyblocks' ) ) :
    ...
    {{#each content}}
        <li><a href="{{item_link}}">{{item_text}}</a>
            {{#if item_description}}<div class=\'item__text\'>{{item_description}}</div>{{/if}}
    </li>
    {{/each}}
    </ul>
        </div>
...
if ( function_exists( 'lazyblocks' ) ) :
    ...
    lazyblocks()->add_block( array(
        ...
        <div class="cblock_address">{{{cblock_address}}}</div>
        <p>{{cblock_phone}}</p>
        {{#if cblock_link}}
            <a href="{{cblock_link}}">{{cblock_link_text}}</a>
        {{/if}}
        {{#if cblock_email}}
            <a href="mailto:{{cblock_email}}">{{cblock_email}}</a>
        {{/if}}
        </div>
        {{/each}}
        </div>
    ...
    <tr>
        <td class="module__text deadlines__date">{{deadlines_date}}</td>
            ...
            <td class=\'item__desc\'>{{{deadlines_description}}}</td>
            {{/if}}
    </tr>    
    {{/each}}
    </table>
    ...
    lazyblocks()->add_block( array(
        ...
        'code' => array(
            ...
            'editor_callback' => '',
            'editor_css' => '',
            'frontend_html' => '<div class="module module--box module--hours">
        <h2 class="module__title">Hours.</h2>
        <div class="content">
    
    {{#each time_period}}
    <div class="module__subsection">
        <h3 class="module__subheader">{{time_period_title}}</h3>
        
        <div class="module__text hours--time">{{time_period_hours}}</div>
            {{#if time_period_description}}
            <div class=\'item__desc\'>{{time_period_description}}</div>
            {{/if}}
    </div>    
